...
CREATE TABLE IF NOT EXISTS `sendmailwrapper`.`messages` (
  ...
  `from` VARCHAR(255) NOT NULL DEFAULT '' COMMENT 'from address',
  `to` TEXT NOT NULL DEFAULT '' COMMENT 'to address(es)',
  `cc` TEXT NOT NULL DEFAULT '' COMMENT 'Cc address(es)',
  `bcc` TEXT NOT NULL DEFAULT '' COMMENT 'Bcc address(es)',
